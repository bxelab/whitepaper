...
另外,为了防止BRC-20代币销毁时对其UTXO关联聪的销毁，BxE协议还定义了一个BTC-ERC20合约，当任意数量的BTC被销毁时，BTC-ERC20合约可以mint对应数量的WBTC实现了BTC的跨协议流转。
...
contract BitcoinERC20 {
    ...
    //mint 传入销毁BTC的交易哈希，系统判断从比特币网络销毁的BTC后，可在BxE上铸造对应数量的WBTC
...
BxE支持单向的BRC-20 Token转移到BxE协议上，以ERC20 Token的形式存在。
以知名的BRC-20 ORDI为例，跨到BxE协议的过程如下：
1. 编写并部署一个满足ERC20规范的代币合约WORDI到BxE上。该合约初始发行量为0，具有mint方法。
2. 用户A持有一定数量m的ORDI，构造ORDI的转账交易，将n个ORDI转移到销毁地址，并记录下该销毁ORDI的交易哈希。
3. 在BxE上发起调用WORDI合约的mint方法的交易，将上一步销毁ORDI的交易哈希作为参数传入。
4. 用户的mint交易被打包到比特币网络，BxE节点执行合约合约mint逻辑：
    a. 判断交易哈希是否已经在之前被处理，如果处理过则返回错误。 
    b. 跨合约调用BRC-20系统合约0x0000000000000000000000000000000000000003的getTransfer方法，获得销毁交易的发起人、销毁Token的数量、销毁Token的Symbol。
    c. 判断销毁的Token的Symbol是不是ORDI，不是则返回错误。
    d. 判断Token的收款人地址是不是销毁地址，不是则返回错误。
    e. 调用ERC20自身的铸币方法`function _mint(address to, uint256 amount)`，为销毁交易的发起人铸造对应销毁数量的WORDI Token。
...
flowchart TD
    ...
    D -->|是ORDI| F{判断收款人地址}
    F -->|不是销毁地址| E[返回错误]
    F -->|是销毁地址| H[调用_mint铸造WORDI]
    H --> End((完成))
```

## 附录B BTC跨到BxE的过程
BxE支持单向的BTC转移到BxE协议上，以ERC20 Token（WBTC）的形式存在。
以下是BTC跨到BxE协议的过程：
1. 用户A持有一定数量m的BTC，构造BTC的转账交易，将n个BTC转移到销毁地址，并记录下该销毁BTC的交易哈希。
2. 在BxE上发起调用BTC-ERC20系统合约的mint方法的交易，将上一步销毁BTC的交易哈希作为参数传入。
3. 用户的mint交易被打包到比特币网络，BxE节点执行合约合约mint逻辑：
    a. 判断交易哈希是否已经在之前被处理，如果处理过则返回错误。 
    b. 跨合约调用BTC系统合约0x0000000000000000000000000000000000000001的getTransaction方法，获得销毁交易的发起人地址、收款人地址、销毁BTC的数量。
    c. 判断BTC的收款人地址是不是销毁地址，不是则返回错误。
    d. 判断BTC的销毁数量是否大于零，不是则返回错误。
    e. 调用自身的铸币方法`function _mint(address to, uint256 amount)`，为销毁交易的发起人铸造对应销毁数量的WBTC。
